'''
This is the new rectified version of the pseudo-document sampling.
This version uses 3K and adds the pseudo document sampling
'''
import numpy as np
...
class APSEN:
    ...
    def __init__(self,qry,fil,K,itr,pseudo,typ):
        ...
        self.lambda_=0.1
        # pseudo document prior
        self.sigma=0.1
        # prior for \phi^{B}
        delta_=0.001
        # the pseudo document to actual document count
        ...
        # topic document count of the pseudo-document
        ...
        ''' The following are the count variables for
         ...
         document part. Instead, we assume every document d is a pseudo document so d=l'''
    ...
    def rand_assign(self):
        ...
        z==1'''
        # randomly initialize document-pseudo document count
        for d in xrange(self.t_docs):
            rl=np.random.randint(self.l)
            # update the lookup table
            self.lookup_d_l[d]=[rl,rl]
            # update the pseudo-doc and document count
            self.n_l_d[rl,d]+=1
        ...
        for d,v in izip(self.td.row,self.td.col):
            ...
            # get the corresponding pseudo document
            ...
            if r==1:
                ...
                document (in our case a sentence) and (2) n_z_l for the pseudo-document.
                Both are required for the pseudo-document inference. '''
                self.n_z_d[z,d]+=1
                self.n_z_l[z,rl]+=1
                self.nz[z]+=1
                self.nd[d]+=1
    ...
    def __getSpikeNSlab(self,l,z):
        ...
        # get the set of non-zero entries for the pseudo-document l and topics K (the identity vector)
        I_c=[t[0] for t in zip(*self.n_z_cl[:,l].nonzero())]
        # zS implies we sum over all z, equivalent of n_z*_l
        n_zS_l=np.sum(self.n_z_l[:,l])
        A_l=len(I_c)
    ...
    def inference(self):
        ...
        for itr in xrange(self.itr_cnt):
            ...
            # (1) begin by sampling for the pseudo-document l for all documents
            for d in xrange(self.t_docs):
                # get the randomly assigned pseudo document
                l=self.lookup_d_l[d][1]
                self.n_l_d[l,d]-=1
                # get the new pseudo document samples for document d
                tmp_l=self.__get_pseudo(d)
                new_l=np.random.multinomial(1,tmp_l).argmax()
  
                # update the pseudo-document count with the newly sampled l
...
if __name__ == '__main__':
    ...
    parser.add_option('-k',dest='topics', default=70, type='int', help='number of topics')
    parser.add_option('-p', dest='pseudo', default=250, type='int', help='number of pseudo documents')
    parser.add_option('-i', dest='iter', default=350, type='int', help='number of iterations')
